                    ...
                    if not name == True:
                        if 'name: ' in line:
                            if not ',' in line:
                                        ...
                                        ini = line.split('name:')[1]
                                        fs.write("exports.code = function(){ \r")
                                        fs.write("    client.on('message', message => {\n")
                                        fs.write("            if (message.content.toLowerCase() === " + f"'{ini}".replace("c_","").replace(" ","").lower().rstrip() + "') { \n")
                                        fs.write("              if(message.author.bot) return; \n")
                                        fs.write("              message.channel.send(" + f"{ini}".replace(" ","").rstrip() + "); \n")
                                        fs.write("	  });\n")
                                        fs.write(f"const {ini} ".replace(" ","").rstrip() + " = new Discord.MessageEmbed()")
                                        fs.write(" \n   .setColor('#1500f7') \n")
                                        fs.write("   .setAuthor('Lemons#5444', 'https://i.imgur.com/9wxw0Sg.png') \n")
